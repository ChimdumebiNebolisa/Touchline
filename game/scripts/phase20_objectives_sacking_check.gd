...
func _process(_delta: float) -> bool:
    ...
    if not world_generator.BeginNewCareer("Phase 20 Sacking Check", 202020, "Manager", "Crisis Interim", "National C License"):
    ...
    var validation_message := str(game_state.ValidatePhase20ObjectivesSackingContract())
    ...
    if not world_generator.BeginNewCareer("Phase 20 Mutation", 202121, "Manager", "Unknown Upstart", "National C License"):
        _fail(world_generator.LastStatusMessage)
        return false
    ...
    var stored_message := str(game_state.ValidatePhase20StoredObjectivesSackingContract())
    ...
    if career_market_label == null:
        _fail("Dashboard career/objective label is missing")
        return false
    var text := str(career_market_label.text)
    if text.find("Objective reviews") == -1 or text.find("Objective history") == -1 or text.find("Sacking history") == -1 or text.find("Sacked") == -1:
        _fail("Dashboard objective/sacking summary is incomplete: %s" % text)
        return false

    print("PHASE20_OBJECTIVES_SACKING_PASS")
    quit()
    return false

func _fail(message: String) -> void:
    push_error(message)
    print("PHASE20_OBJECTIVES_SACKING_FAIL: " + message)
    quit(1)
