...
func _process(_delta: float) -> bool:
    ...
    for role in ["Assistant Manager", "Head Coach", "Manager"]:
        if not world_generator.BeginNewCareer("Phase 12 Staff Check", 121212, role, "Unknown Upstart", "National C License"):
            _fail(world_generator.LastStatusMessage)
            return false
        if not world_generator.SelectClub("Northbridge City"):
            _fail(world_generator.LastStatusMessage)
            return false
        var validation_message := str(game_state.ValidatePhase12StaffImpactMarketContract())
    ...
    if not world_generator.BeginNewCareer("Phase 12 Staff Mutation", 121312, "Manager", "Unknown Upstart", "National C License"):
        _fail(world_generator.LastStatusMessage)
        return false
    ...
    var stored_message := str(game_state.ValidatePhase12StoredStaffImpactMarketContract())
    if stored_message != "OK":
        _fail(stored_message)
        return false

    print("PHASE12_STAFF_MARKET_PASS")
    quit()
    return false

func _fail(message: String) -> void:
    push_error(message)
    print("PHASE12_STAFF_MARKET_FAIL: " + message)
    quit(1)
