...
func _process(_delta: float) -> bool:
    ...
    for role in ["Assistant Manager", "Head Coach", "Manager"]:
        if not world_generator.BeginNewCareer("Phase 10 Contract Check", 101010, role, "Unknown Upstart", "National C License"):
            _fail(world_generator.LastStatusMessage)
            return false
        if not world_generator.SelectClub("Riverton Athletic"):
            _fail(world_generator.LastStatusMessage)
            return false
        var validation_message := str(game_state.ValidatePhase10ContractNegotiationContract())
    ...
    if not world_generator.BeginNewCareer("Phase 10 Contract Mutation", 101110, "Manager", "Unknown Upstart", "National C License"):
        _fail(world_generator.LastStatusMessage)
        return false
    ...
    var stored_message := str(game_state.ValidatePhase10StoredContractNegotiationContract())
    if stored_message != "OK":
        _fail(stored_message)
        return false

    print("PHASE10_CONTRACT_NEGOTIATION_PASS")
    quit()
    return false

func _fail(message: String) -> void:
    push_error(message)
    print("PHASE10_CONTRACT_NEGOTIATION_FAIL: " + message)
    quit(1)
