...
func _process(_delta: float) -> bool:
    ...
    if not world_generator.BeginNewCareer("Phase 15 Finance Check", 151515, "Manager", "Unknown Upstart", "National C License"):
    ...
    var validation_message := str(game_state.ValidatePhase15FinanceContract())
    ...
    if not world_generator.BeginNewCareer("Phase 15 Finance Mutation", 151616, "Manager", "Unknown Upstart", "National C License"):
        _fail(world_generator.LastStatusMessage)
        return false
    ...
    var stored_message := str(game_state.ValidatePhase15StoredFinanceContract())
    ...
    if career_market_label == null:
        _fail("Dashboard career/finance label is missing")
        return false
    var text := str(career_market_label.text)
    if text.find("Finance") == -1 or text.find("Transfer budget remaining") == -1 or text.find("wage bill") == -1:
        _fail("Dashboard finance summary is incomplete: %s" % text)
        return false

    print("PHASE15_FINANCE_PASS")
    quit()
    return false

func _fail(message: String) -> void:
    push_error(message)
    print("PHASE15_FINANCE_FAIL: " + message)
    quit(1)
