...
func _process(_delta: float) -> bool:
    ...
    if not world_generator.BeginNewCareer("Phase 21 Job Market Check", 212121, "Manager", "Former Club Legend", "National B License"):
    ...
    var validation_message := str(game_state.ValidatePhase21CareerJobMarketContract())
    ...
    if not world_generator.BeginNewCareer("Phase 21 Mutation", 212222, "Assistant Manager", "Unknown Upstart", "Grassroots License"):
        _fail(world_generator.LastStatusMessage)
        return false
    ...
    var stored_message := str(game_state.ValidatePhase21StoredCareerJobMarketContract())
    ...
    if career_market_label == null:
        _fail("Dashboard career/job label is missing")
        return false
    var text := str(career_market_label.text)
    if text.find("Job market history") == -1 or text.find("Accepted offer") == -1 or text.find("License course") == -1:
        _fail("Dashboard career job-market summary is incomplete: %s" % text)
        return false

    print("PHASE21_CAREER_JOB_MARKET_PASS")
    quit()
    return false

func _fail(message: String) -> void:
    push_error(message)
    print("PHASE21_CAREER_JOB_MARKET_FAIL: " + message)
    quit(1)
