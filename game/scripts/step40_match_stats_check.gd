...
func _process(_delta: float) -> bool:
    ...
    game_state.StartNewCareer("Stats Check", 556677)
    game_state.SelectClub("Riverton Athletic")

    var validation_message := str(game_state.ValidateMatchStatsContract())
    if validation_message != "OK":
        _fail(validation_message)
        return false

    print("STEP40_MATCH_STATS_PASS")
    quit()
    return false

func _fail(message: String) -> void:
    push_error(message)
    print("STEP40_MATCH_STATS_FAIL: " + message)
    quit(1)
