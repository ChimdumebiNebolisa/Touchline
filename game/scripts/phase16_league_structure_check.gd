...
func _process(_delta: float) -> bool:
    ...
    if not world_generator.BeginNewCareer("Phase 16 League Check", 161616, "Manager", "Unknown Upstart", "National C License"):
    ...
    var validation_message := str(game_state.ValidatePhase16LeagueStructureContract())
    ...
    if not world_generator.BeginNewCareer("Phase 16 League Mutation", 161717, "Manager", "Unknown Upstart", "National C License"):
        _fail(world_generator.LastStatusMessage)
        return false
    ...
    var stored_message := str(game_state.ValidatePhase16StoredLeagueStructureContract())
    ...
    if career_market_label == null:
        _fail("Dashboard career/league label is missing")
        return false
    var text := str(career_market_label.text)
    if text.find("League system") == -1 or text.find("Tier") == -1 or text.find("League history") == -1:
        _fail("Dashboard league structure summary is incomplete: %s" % text)
        return false

    print("PHASE16_LEAGUE_STRUCTURE_PASS")
    quit()
    return false

func _fail(message: String) -> void:
    push_error(message)
    print("PHASE16_LEAGUE_STRUCTURE_FAIL: " + message)
    quit(1)
