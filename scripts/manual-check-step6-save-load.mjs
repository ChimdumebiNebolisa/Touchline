...
async function main() {
  ...
  const slotRootDirectory = await mkdtemp(join(tmpdir(), "touchline-step6-manual-"));
  let slotEnvelope;
  let slotList;
  try {
    await writeSaveSlot({
      rootDirectory: slotRootDirectory,
      slotId: "manual-slot",
      state: saveState,
      savedAtIso: "2033-02-03T09:00:00.000Z"
    });
    slotEnvelope = await readSaveSlot({ rootDirectory: slotRootDirectory, slotId: "manual-slot" });
    slotList = await listSaveSlots(slotRootDirectory);
  } finally {
    await rm(slotRootDirectory, { recursive: true, force: true });
  }

  const continuedFromSlot = advanceSeasonState(slotEnvelope.state.worldState, majorEventResults);
  ...
  console.table([
    {
      ...
      slotNextMatchday: continuedFromSlot.currentMatchday,
      ...
      slotCompletedFixtureCount: continuedFromSlot.completedFixtureIds.length
    }
  ]);
  ...
  console.table([
    {
      ...
      slotId: slotList[0]?.slotId ?? "none",
      slotVersion: slotList[0]?.version ?? -1
    }
  ]);
  ...
  const checks = {
    ...
    deterministicContinuationAfterSlotReload:
      JSON.stringify(continuedFromSlot) === JSON.stringify(continuedFromOriginal),
    savesManagerCareerState: savesFullCareerState,
    slotPersistenceWorks: slotList.length === 1 && slotEnvelope.savedAtIso === envelope.savedAtIso
  };
